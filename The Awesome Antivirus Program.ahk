...
if not FileExist("avp.ini")
{
    IniWrite, 0,avp.ini,Settings, startup
}


IniRead, startup, avp.ini, Settings,startup
...
menu, tray, add, Startup, startup
if startup
	menu, tray, Check, startup
...
ShellMessage( wParam,lParam ) { 
    ...
    { 
		WinGetClass, out , ahk_id %lParam%
		if out=IEFrame
		{

			if GetKeyState("ctrl")
			{	
				 NotifyID := Notify("","Internet Explorer window allowed.",2,"GC=black TC")
				 safelist=%safelist%|%lParam%
				return
			}
			if timeout
				return
			winkill, ahk_id %lParam%
		
			Notify("The Awesome Antivirus Program","The Awesome Antivirus Program has prevented Internet Explorer from running.`nClick this window to allow it to be opened.",4,"AC=ActionDemo ax=1  GC=White IN=8 IW=48 IH=48", A_WinDir "\explorer.exe")
		}
...
ActionDemo:
 NotifyID := Notify("","Internet Explorer temporarily allowed to run.",5)
 	 safelist=%safelist%|%window%

 timeout=1
 settimer, timeoff, 5000
Return
...
startup:
	 if startup=1
    {
		...
		regDelete, HKCU,Software\Microsoft\Windows\CurrentVersion\Run,avp
		startup=0
		    IniWrite, 0,avp.ini,Settings, startup
			menu, tray, UnCheck, Startup
	}
	Else
	{
		RegWrite, REG_SZ,HKCU,Software\Microsoft\Windows\CurrentVersion\Run,avp, "%A_ScriptFullPath%"
		startup=1
		 IniWrite, 1,avp.ini,Settings, startup
		 menu, tray, Check, Startup
	}
return
